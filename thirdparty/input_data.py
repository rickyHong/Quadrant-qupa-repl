...
class DataSet(object):
    def __init__(self, images, should_binarize=False, one_hot=False):
        ...
        self.index = np.arange(0, self._num_examples)
    ...
    def next_batch(self, batch_size):
        ...
        return batch.astype(numpy.float32), None, index
...
def read_binarize_mnist(train_dir, get_subset):
    data_sets = DataSets()
...
def read_omniglot(data_dir, get_subset, binarize_val_test=False):
...
if __name__ == '__main__':
    ...
    from util import get_structure_mask
    dataset = read_data_set('../data', dataset='binarized_mnist', get_subset=True)
    image = dataset.train.images[0]
    mask = get_structure_mask(784)
    image = image * (1 - mask)
